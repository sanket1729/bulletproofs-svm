...
def process_cand_inner(w,xi,n):
    ...
    inner_product_verifier(n,V,prf2)

def process_candidate(w, xi, n):
    rangebits = 128
    w_vec = Vector(w)
    xi_vec = Vector(xi)
    value = xi_vec.inner_product(w_vec)
    ...
    # Process the V from the range proof
    pc = PC(value, blinding=rp.gamma)
    assert pc.get_commitment() == rp.V
    ...
    r2 = (decode(rp.gamma, 256)*e + decode(r0,256)) %N

    r2_enc = encode(r2, 256, 32)
    final_pc = PC(blinded_final_product, blinding = r2)

    lhs = final_pc.get_commitment()
    rhs = ecmult(e, rp.V, False)
    rhs_2 = pc2.get_commitment()
    rhs = ecadd_pubkeys([rhs,rhs_2], False)
